...
static bool pcapng_write_idb(struct xpcapng_dumper *pd, const char *name,
                 uint32_t snap_len, const char *description,
                 ...
                 struct xpcapng_epb_options_s *epb_options)
{
    ...
    struct {
        ...
        uint16_t pph_len;
        uint32_t pph_dlt;
    } ppi_header = { 0, 0, 8, epb_options->ppi_linktype };
    if (epb_options->ppi_linktype) {
        iov[i].iov_base = &ppi_header;
        iov[i++].iov_len = 8;
        caplen -= 8;
    }
 ...
 * xpcapng_dump_open()
 *****************************************************************************/
struct xpcapng_dumper *xpcapng_dump_open(FILE *f,
                     const char *comment,
                     const char *hardware,
                     const char *os,
                     const char *user_application)
 ...
 *****************************************************************************/
int xpcapng_dump_add_interface(struct xpcapng_dumper *pd, uint32_t snap_len,
                   ...
                   uint16_t link_type)
{
    if (!pcapng_write_idb(pd, name, snap_len, description, mac, speed,
                  ts_resolution, hardware, link_type))
        return -1;
